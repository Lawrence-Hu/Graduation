...
//    }
    ...
    void print(String s,int len) throws UnsupportedEncodingException {
        char[] chars = s.toCharArray();
        for (int i = 0; i < chars.length; i++) {
            if(len>= String.valueOf(chars[i]).getBytes("GBK").length){
                System.out.print(chars[i]);
                len-=String.valueOf(chars[i]).getBytes("GBK").length;
            }
        }
    }
}